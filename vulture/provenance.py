...
class Provenance(object):
    ...
    def start(self, workflow=False):
        ...
        from vulture import __version__ as vulture_version
        ...
        self.sw_vulture = self.doc.agent(
            ":vulture",
            {
                "prov:type": "prov:SoftwareAgent",
                "dcterms:source": f"https://github.com/cedadev/vulture/releases/tag/v{vulture_version}",
            },
        )
        self.doc.wasAttributedTo(self.sw_vulture, project_cds)
        ...
        if workflow is True:
            ...
            self.doc.wasAssociatedWith(
                orchestrate, agent=self.sw_vulture, plan=self.workflow
            )
    ...
    def add_operator(self, operator, parameters, collection, output):
        ...
        if self.workflow:
            ...
            self.doc.start(op, starter=self.sw_daops, trigger=self.sw_vulture)
